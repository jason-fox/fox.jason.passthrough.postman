...
function addSampleCurl(request) {
  ...
  if (request.method === 'HEAD') {
    ...
    snippet.push(
      'curl -X ' + (request.url.raw.indexOf('?') ? ' -G ' : '') + request.method
    );
    snippet.push("'" + getURL(request.url.raw) + "'");
  } else {
  ...
  if (request.url.raw.indexOf('?') > 0) {
    var params = request.url.raw
      .substring(request.url.raw.indexOf('?') + 1)
      .split('&');
...
function addEndpoint(request) {
  ...
  lines.push(request.method.toUpperCase() + ' ' + getURL(request.url.raw));
  lines.push('```');
  lines.push('');
}
